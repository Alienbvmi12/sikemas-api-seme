  ...
  **/
	...
	class WideImage
	{
		...
		static function loadFromFile($uri)
		{
            ...
            $file_handler = fopen($uri, 'r');
            $data = fread($file_handler, filesize($uri));
            fclose($file_handler);
		...
		{
			return ((is_resource($handle) && get_resource_type($handle) == 'gd') || (is_object($handle) && get_class($handle) == 'GdImage'));
		}
