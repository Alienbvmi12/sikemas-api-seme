	...
	$("#fmodal_profil_foto").on("submit", function(e) {
		...
		NProgress.start();
		$.ajax({
			url: '<?= base_url('api_front/profil/edit_foto/'); ?>', // Url to which the request is send
			type: "POST",
	...
	$("#fmodal_profil_edit").on("submit", function(e) {
		...
		$.post('<?= base_url('api_front/profil/edit_profil') ?>', fd).done(function(dt) {
			NProgress.set(0.4);
			if (dt.status == 200) {
				NProgress.done();
				gritter("<h4>Sukses</h4><p>profile telah diubah</p>", 'success');
				const tabelProfil = document.getElementById("profile-table").childNodes[1].childNodes;
				console.log(tabelProfil);
				tabelProfil[0].childNodes[5].innerHTML = dt.data.fnama + " " + dt.data.lnama;
				tabelProfil[4].childNodes[5].innerHTML = dt.data.telp;
				$("#imodal_profil_edit_fnama").val(dt.data.fnama);
				$("#imodal_profil_edit_lnama").val(dt.data.lnama);
				$("#imodal_profil_edit_telp").val(dt.data.telp);
				$("#modal_profil_edit").modal('hide');
			} else {
				NProgress.done();
				gritter("<h4>Galat</h4><p>" + dt.message + "</p>", 'danger');
			}
		}).fail(function() {
			NProgress.done();
	...
	$("#fmodal_password_change").on("submit", function(e) {
		e.preventDefault();
		var psl = $("#imodal_password_change_oldpassword").val();
		...
		$.post('<?= base_url('api_front/profil/password_change/') ?>', fd).done(function(dt) {
			...
			if (dt.status == 200) {
				...
				setTimeout(function() {
					window.location = '<?= base_url('front/profil/') ?>';
				}, 2000);
			} else {
...
<?php  $this->getThemeElement('profil/alamat_bottom', $__forward); ?>
