...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		NProgress.start();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_front/advis/")?>",
          ...
          $('#drTable > tbody').off('click', 'tr');
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
            $('#adetail').attr('href','<?=base_url("advis/detail/")?>'+encodeURIComponent(ieid));
						$('#aedit').attr('href','<?=base_url("advis/edit/")?>'+encodeURIComponent(ieid));
