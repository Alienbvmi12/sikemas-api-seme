...
function updateFile(sinput=""){
  ...
  $.ajax({
      url: '<?=base_url("api_front/rekomendasi/update_file/")?>',
      type: 'post',
      data: fd,
      contentType: false,
      processData: false,
...
function postFile(sinput=""){
  ...
  fd.append('utype',utype);
  $.ajax({
    url: '<?=base_url("api_front/rekomendasi/upload_file/")?>',
    type: 'post',
    data: fd,
    contentType: false,
    processData: false,
    success: function(response){
      ...
      $("#panelfile_"+name).on('click','.delete-file', function(e){
        ...
        $.get('<?=base_url("api_front/rekomendasi/delete_lampiran/")?>'+id).done(function(dt){
          if(dt.status == 200){
...
function postData(sinput){
  ...
  var url = '<?=base_url("api_front/rekomendasi/baru/"); ?>';
  fd.append('rekomendasi_id',rekomendasi_id);
  $.ajax({
    type: 'post',
    url: url,
    data: fd,
    ...
    success: function(respon){
      if(respon.status==200){
        if(is_submit == 1){
          ...
          setTimeout(function(){
            window.location = '<?=base_url('rekomendasi')?>';
          }, 2666);
        }else{
          $('.btn-submit').prop('disabled',false);
          $('.icon-submit').removeClass('fa-spin');
...
$('input[type="file"]').on('change', function(e){
  ...
  if(rekomendasi_id == '' || rekomendasi_id == 0){
    postData(sinput);
  }else{
