...
<script>
	...
	$("#form-register").on("submit", function(evt) {
		...
		var url = '<?= base_url('daftar/verifyRegisterToken') ?>';
	...
	function resendCode() {
		$("#kirim-ulang").prop('disabled', true);
		var count = 3;
		$("#countDown").text("  "+count+"s");
		var interval = setInterval(function(){
			if(count === 0){
				clearInterval(interval);
				$("#kirim-ulang").prop('disabled', false);
				$("#countDown").text("");
			}
			else{
				count--;
		...
		$.ajax({
			url: '<?= base_url('daftar/regenerateToken') ?>',
			type: 'GET',
			dataType: 'json',
			contentType: 'application/json',
			success: function(response) {
				if (response.status == 200) {
					gritter(response.message, 'success');
					console.log(response.message);
				} else {
					gritter(response.message, 'danger');
					console.log(response.message);
				}
			},
			error: function(request, status, error) {
				gritter(request, "error");
				console.log(request);
			}
		});
	}
