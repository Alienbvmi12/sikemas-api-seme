...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		"bProcessing"		: true,
		"bServerSide"		: true,
		"sAjaxSource"		: "<?=base_url("api_admin/erpmaster/kelasrombel/"); ?>",
		...
		"fnServerData": function (sSource, aoData, fnCallback, oSettings) {
			oSettings.jqXHR = $.ajax({
				...
				$('#drTable > tbody').on('click', 'tr', function (e) {
					...
					ieid = id;
					$("#aedit").attr("href",'<?=base_url_admin('erpmaster/kelasrombel/edit/')?>'+ieid);
					$("#adetail").attr("href",'<?=base_url_admin('erpmaster/kelasrombel/detail/')?>'+ieid);
...
$("#ahapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			$().btnSubmit();
			var url = '<?=base_url('api_admin/erpmaster/kelasrombel/hapus/'); ?>'+ieid;
