...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/erpmaster/gurutendik_jabatan/"); ?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						$("#adetail").attr("href","<?=base_url_admin("erpmaster/gurutendik_jabatan/detail/")?>"+ieid);
						$("#aedit").attr("href","<?=base_url_admin("erpmaster/gurutendik_jabatan/edit/")?>"+ieid);
					});
	...
	$('.dataTables_filter input').attr('placeholder', 'Cari nama, kode');
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			...
			$('.btn-submit').prop('disabled',true);
			$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			var url = '<?=base_url('api_admin/erpmaster/gurutendik_jabatan/hapus/')?>'+ieid;
