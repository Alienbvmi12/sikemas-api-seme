...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		"bProcessing"		: true,
		"bServerSide"		: true,
		"sAjaxSource"		: "<?=base_url("api_admin/erpmaster/gurutendik/"); ?>",
		"fnServerParams"	: function ( aoData ) {
			aoData.push(
				{ "name": "a_institusi_id", "value": $("#filter_a_institusi_id").val() },
				{ "name": "b_user_id", "value": $("#filter_b_user_id").val() },
		...
		"fnServerData": function (sSource, aoData, fnCallback, oSettings) {
			oSettings.jqXHR = $.ajax({
				...
				$('#drTable > tbody').on('click', 'tr', function (e) {
					...
					ieid = id;
					$("#aedit").attr("href",'<?=base_url_admin('erpmaster/gurutendik/edit/')?>'+ieid);
					$("#adetail").attr("href",'<?=base_url_admin('erpmaster/gurutendik/detail/')?>'+ieid);
...
$("#ahapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			$().btnSubmit();
			var url = '<?=base_url('api_admin/erpmaster/gurutendik/hapus/'); ?>'+ieid;
