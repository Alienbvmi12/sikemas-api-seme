...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		...
		"bProcessing"		: true,
		"bServerSide"		: true,
		"sAjaxSource"		: "<?=base_url("api_admin/erpmaster/pengguna_jabatan/"); ?>",
		"fnServerParams"	: function ( aoData ) {
			aoData.push(
				{ "name": "a_jabatan_id", "value": $("#filter_a_jabatan_id").val() },
		...
		"fnServerData": function (sSource, aoData, fnCallback, oSettings) {
			oSettings.jqXHR = $.ajax({
				...
				$('#drTable > tbody').on('click', 'tr', function (e) {
					...
					ieid = id;
					$("#aedit").attr("href",'<?=base_url_admin('erpmaster/pengguna_jabatan/edit/')?>'+ieid);
					$("#adetail").attr("href",'<?=base_url_admin('erpmaster/pengguna_jabatan/detail/')?>'+ieid);
...
$("#ahapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			$().btnSubmit();
			var url = '<?=base_url('api_admin/erpmaster/pengguna_jabatan/hapus/'); ?>'+ieid;
